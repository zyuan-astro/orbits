...
def createbarPotential(A, b, v_c, r_cr, omega_p, m, phi_b, t=0):
    '''
    Create a CylSpline approximation for a bar potential from Chiba&Schönrich 2021.
    The density
    '''
    def potfnc(xyz):
        ...
        Phi_b = -0.5 * (A * v_c**2) * (r/r_cr)**2 * ((b + 1)/(b + (r/r_cr)))**5
        Phi = Phi_b * (numpy.sin(theta)**2) * numpy.cos(m * (phi - omega_p * t - phi_b))
    ...
    Rmin = 0.1,  # (these could be adjusted if needed)
    Rmax = 40,
    zmin = 0.05,
    zmax = 20,
    mmax = 12,
...
omega_p = -35
m1 = 2
phi_b = numpy.radians(28)
#m2 = 4
#m3 = 6
pot_bar_1 = createbarPotential(A, b, v_c, r_cr, omega_p, m1, phi_b, t=0)
#pot_bar_2 = createbarPotential(A, b, v_c, r_cr, omega_p, m2, t=0)
#pot_bar_3 = createbarPotential(A, b, v_c, r_cr, omega_p, m3, t=0)
pot_bar = agama.Potential(pot_bar_1)
pot_back = model.potential
pot = agama.Potential(pot_bar, pot_back[1], pot_back[0])

Omega_p = -35
Omega_f = Omega_p * 0.45
#t_p = 0.5  # time at which the potential starts to slow down, again in our time units
#t_f = 4.0  # time at which it achieves the final pattern speed
...
print('Pattern speed is %.3g, %.3g' %(pattern_slow[0],pattern_slow[1]))
print(rot_angle_spl([-6,-3,0],der=1))
numpy.savetxt('/place/to/constbar_sp/rot_angle.txt', rot_angle)
    
pot_bar_slowing_growing = agama.Potential(potential=pot_bar,
            rotation='/place/to/constbar_sp/rot_angle.txt')
...
numpy.savetxt('/place/to/constbar_sp/rot_angle_sp.txt', rot_angle_sp)


pot_spiral_rot = agama.Potential(potential=pot_spiral, 
                                 rotation='/place/to/constbar_sp/rot_angle_sp.txt')
...
numpy.save('/place/to/constbar_sp/orbits',orbit_final)
